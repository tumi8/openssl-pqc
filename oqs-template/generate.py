...
def populate(filename, config, delimiter, overwrite=False):
    ...
    generatehelpers.file_put_contents(filename, contents)

def load_config(include_disabled_sigs=False):
    config = generatehelpers.file_get_contents(os.path.join('oqs-template', 'generate.yml'), encoding='utf-8')
...
config = load_config()
config = generatehelpers.complete_config(config)
...
print("Code generation complete. Be sure to run `make generate_crypto_objects` if required.")
